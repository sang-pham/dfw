...
const getRouterByOption = (options) => {
  ...
  if (options['firewallName'] && !options['firewallIp']) {
    routerNameList = options['firewallName'].split(',').filter(item => !!item)
  ...
  } else if (!options['firewallName'] && options['firewallIp']) {
    routerIpList = options['firewallIp'].split(',').filter(item => !!item)
